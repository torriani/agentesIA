#!/bin/bash
# ig-text: Extrai transcricao de video do Instagram + formata com Gemini Flash 2.5
# Tenta captions primeiro; se nao existir, baixa audio e transcreve com Whisper
...
show_help() {
  cat <<'HELP'
=== ig-text - Transcricao Instagram via Captions/Whisper + Gemini ===

Uso: ig-text.sh <url-instagram>

Parametros:
  url         URL do video/reel do Instagram (obrigatorio)

Comportamento:
  - Tenta extrair captions/legendas nativas (se existirem)
  - Se nao houver captions: baixa audio e transcreve com Whisper
  - Traduz e formata tudo em portugues (pt-BR) via Gemini

Exemplos:
  ig-text.sh "https://www.instagram.com/reel/abc123/"
  ig-text.sh "https://www.instagram.com/p/abc123/"
HELP
}
...
# whisper is optional — checked at runtime if needed
...
if [[ ! "$URL" =~ (instagram\.com) ]]; then
  echo "ERRO: URL nao parece ser do Instagram."
  echo "  Formatos aceitos:"
  echo "    https://www.instagram.com/reel/xxx/"
  echo "    https://www.instagram.com/p/xxx/"
  exit "$EXIT_INPUT_ERROR"
fi
...
draw_dashboard() {
  ...
  printf "${C_BOLD}${C_CYAN}║${C_RESET}  ${C_BOLD}ig-text${C_RESET} ${C_DIM}— Transcricao Instagram + Gemini Flash 2.5${C_RESET}            ${C_CYAN}║${C_RESET}\n"; ((lines++))
  printf "${C_BOLD}${C_CYAN}╠══════════════════════════════════════════════════════════════════╣${C_RESET}\n"; ((lines++))
...
# --- STEP 2: Legendas (tentar captions, fallback Whisper) ---
...
# --- Tentativa 1: Captions nativas (raro no Instagram, mas vale tentar) ---
...
if [ -n "$SUBTITLE_FILE" ]; then
  # Captions encontradas — processar VTT
  ...
  # --- Tentativa 2: Fallback Whisper (baixar audio + transcrever) ---

  # Verificar se whisper esta instalado
  WHISPER_BIN=""
  if command -v whisper &>/dev/null; then
    WHISPER_BIN="whisper"
  elif [ -f "$HOME/Library/Python/3.9/bin/whisper" ]; then
    WHISPER_BIN="$HOME/Library/Python/3.9/bin/whisper"
  elif [ -f "$HOME/Library/Python/3.10/bin/whisper" ]; then
    WHISPER_BIN="$HOME/Library/Python/3.10/bin/whisper"
  elif [ -f "$HOME/Library/Python/3.11/bin/whisper" ]; then
    WHISPER_BIN="$HOME/Library/Python/3.11/bin/whisper"
  elif [ -f "$HOME/Library/Python/3.12/bin/whisper" ]; then
    WHISPER_BIN="$HOME/Library/Python/3.12/bin/whisper"
  fi
  ...
  if [ -z "$WHISPER_BIN" ]; then
    ...
    draw_dashboard
    echo ""
    echo "ERRO: Nenhuma legenda encontrada e whisper nao esta instalado."
    echo "  Instale com: pip install openai-whisper"
  ...
  mkdir -p "$AUDIO_DIR"
  register_temp "$AUDIO_DIR"

  # Baixar audio via yt-dlp
  printf "${C_CYAN}║${C_RESET}   ${C_DIM}Sem captions → baixando audio para Whisper...${C_RESET}\n"
  yt-dlp -x --audio-format wav --audio-quality 0 \
    --output "$AUDIO_DIR/audio.%(ext)s" "$URL" 2>/dev/null

  AUDIO_FILE=$(find "$AUDIO_DIR" -name "*.wav" 2>/dev/null | head -1)
  if [ -z "$AUDIO_FILE" ]; then
    # Tentar converter se nao veio em wav
    AUDIO_ANY=$(find "$AUDIO_DIR" -type f 2>/dev/null | head -1)
    if [ -n "$AUDIO_ANY" ]; then
      ffmpeg -y -i "$AUDIO_ANY" -ac 1 -ar 16000 "$AUDIO_DIR/audio_16k.wav" 2>/dev/null
      AUDIO_FILE="$AUDIO_DIR/audio_16k.wav"
    fi
  fi
  ...
  if [ -z "$AUDIO_FILE" ] || [ ! -f "$AUDIO_FILE" ]; then
    ...
    draw_dashboard
    echo ""
    echo "ERRO: Falha ao baixar audio do Instagram."
    exit "$EXIT_PROCESSING_ERROR"
  fi

  # Converter pra 16kHz mono se ainda nao estiver
  AUDIO_16K="$AUDIO_DIR/audio_16k.wav"
  if [ "$AUDIO_FILE" != "$AUDIO_16K" ]; then
    ffmpeg -y -i "$AUDIO_FILE" -ac 1 -ar 16000 "$AUDIO_16K" 2>/dev/null
    AUDIO_FILE="$AUDIO_16K"
  fi

  # Rodar Whisper
  printf "${C_CYAN}║${C_RESET}   ${C_DIM}Transcrevendo com Whisper (modelo base)...${C_RESET}\n"
  WHISPER_OUT_DIR="$AUDIO_DIR/whisper"
  ...
  WHISPER_TXT=$(find "$WHISPER_OUT_DIR" -name "*.txt" 2>/dev/null | head -1)
  if [ -z "$WHISPER_TXT" ] || [ ! -s "$WHISPER_TXT" ]; then
    STEP_2_STATUS="error"
    draw_dashboard
    echo ""
    echo "ERRO: Whisper nao conseguiu transcrever o audio."
    exit "$EXIT_PROCESSING_ERROR"
  fi

  cp "$WHISPER_TXT" "$RAW_TEXT"
fi
...
if [ "$RAW_WORDS" -gt "$CHUNK_THRESHOLD" ]; then
  ...
  for CHUNK_FILE in "$FORMATTED_CHUNKS_DIR"/chunk_*; do
    ...
    {
      echo "Voce e um editor de texto especializado. Recebeu a PARTE $CHUNK_NUM de $TOTAL_CHUNKS da transcricao bruta de um video do Instagram."
      echo ""
...
SUMMARY_BODY
  ...
  {
    echo "Voce e um editor de texto especializado. Recebeu a transcricao bruta de um video do Instagram."
    echo "TODO o output DEVE estar em PORTUGUES BRASILEIRO (pt-BR)."
    echo ""
    echo "TITULO DO VIDEO: $VIDEO_TITLE"
    echo "CANAL: $VIDEO_CHANNEL"
...
if [ "$TRANSCRIPTION_METHOD" = "whisper" ]; then
  printf "${C_GREEN}║${C_RESET}  ${C_BOLD}${C_GREEN}✓ TRANSCRICAO COMPLETA (via Whisper)${C_RESET}                         ${C_GREEN}║${C_RESET}\n"
else
  printf "${C_GREEN}║${C_RESET}  ${C_BOLD}${C_GREEN}✓ TRANSCRICAO COMPLETA (via Captions)${C_RESET}                        ${C_GREEN}║${C_RESET}\n"
fi
